...
} from "../../src/plugin-sdk/memory-core-host-engine-foundation.js";

type IsOptional<T, K extends keyof T> = Pick<T, K> extends Required<Pick<T, K>> ? false : true;

// memory core host engine foundation contracts
// preserves the released resolved-config authoring shapes
...
expectTypeOf<IsOptional<ResolvedMemorySearchConfig, "documentInputType">>().toEqualTypeOf<true>();
expectTypeOf<
  IsOptional<ResolvedMemorySearchConfig, "outputDimensionality">
>().toEqualTypeOf<true>();
expectTypeOf<IsOptional<ResolvedMemorySearchConfig["local"], "modelPath">>().toEqualTypeOf<true>();
expectTypeOf<
  IsOptional<ResolvedMemorySearchConfig["store"]["vector"], "extensionPath">
>().toEqualTypeOf<true>();
expectTypeOf<ResolvedMemorySearchConfig["sync"]["embeddingBatchTimeoutSeconds"]>().toEqualTypeOf<
  number | undefined
>();
...
// preserves the released resolver return contracts
expectTypeOf<
