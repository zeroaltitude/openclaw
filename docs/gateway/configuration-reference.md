...
- `plugins.entries.<id>.hooks.allowConversationAccess`: when `true`, trusted non-bundled plugins may read raw conversation content from typed hooks such as `before_model_resolve`, `agent_turn_prepare`, `before_prompt_build`, `before_agent_reply`, `llm_input`, `llm_output`, `before_agent_run`, `before_agent_finalize`, and `agent_end`. Leaving it unset on a non-bundled plugin that registers one of those hooks blocks the registration and logs an `error` diagnostic, because the plugin cannot detect the refusal itself. Setting it to `false` is treated as a deliberate choice and logs a `warn`. One exception: `allowPromptInjection` is evaluated first, so when it is `false` a refused `agent_turn_prepare` or `before_prompt_build` registration — the two hooks that are both prompt-injection and conversation hooks — is attributed to `plugins.entries.<id>.hooks.allowPromptInjection` and logged at `warn`, even when `allowConversationAccess` is unset. `openclaw plugins inspect <id> --runtime` freshly evaluates the current files and config; `/status plugins` reports blocked registrations retained by the running Gateway.
