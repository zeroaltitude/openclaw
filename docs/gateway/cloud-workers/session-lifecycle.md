...
A failed placement does not always mean its worker has stopped. The sidebar, session list, and placement chip keep **Stop cloud worker…** available when cleanup is still needed. Once the Gateway confirms the worker is gone, those menus hide Stop and the placement chip offers **Restart session…**. Choose **Gateway · local**, an eligible paired device, or a configured cloud profile. Local recovery restores the last accepted workspace checkpoint before enabling local turns, including creating a managed worktree for a repository-only session. Unsynced worker changes may be lost, and the interrupted turn is not replayed. The previous failure clears when restart begins; a failed restart reports its new error. An archived session must be unarchived before continuing.

Archiving or deleting a non-main cloud-worker session automatically stops its worker. The Gateway interrupts and drains current work, cancels an in-flight dispatch, and retries pending cleanup for a failed placement. A provisioning placement left behind by an interrupted dispatch can also be stopped this way. The archive or deletion commits only after the worker's cleanup confirms release; active workers receive a final workspace reconciliation first. Reconciliation already in progress must finish before archiving. If cleanup is unavailable or fails, the operation reports an error and retains the session and recovery state; it never force-discards unsynced work. Resolve any provider error, then retry archive to retry cleanup. Restoring an archived session retains reclaimed placement metadata so the next turn can dispatch a fresh worker with the same workspace profile.
