...
suite.define(() => {
  it("wires settled transcript gestures, escalation, keyboard choices, and visit reset", async () => {
    ...
    try {
      ...
      const offline = page.locator('.agent-chat__composer-status[data-tone="warn"]');
  ...
  it.each(["automatic", "manual"])(
    ...
    async (choice) => {
      ...
      try {
        ...
        await gateway.setOnline(false);
        await pane.locator('.agent-chat__composer-status[data-tone="warn"]').waitFor();
        if (choice === "manual") {
          await card.locator("summary").press("Enter");
          expect(
            await pane.evaluate(
        ...
        await gateway.setOnline(true);
        await pane
          .locator('.agent-chat__composer-status[data-tone="warn"]')
