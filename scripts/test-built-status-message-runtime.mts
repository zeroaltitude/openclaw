 ...
 */
export function findBuiltStatusMessageRuntimePath(distDir: string) {
  ...
  assert.ok(preferred, `missing built status-message runtime bundle under ${distDir}`);
  return path.join(distDir, preferred);
}

function listBuiltStatusMessageRuntimeFiles(distDir: string) {
