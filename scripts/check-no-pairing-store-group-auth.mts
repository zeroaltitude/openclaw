...
function getDeclarationNameText(name: ts.BindingName) {
...
function containsPairingStoreSource(node: ts.Node) {
  let found = false;
...
function getCallName(node: ts.Node) {
...
function isSuspiciousNormalizeWithStoreCall(node: ts.Node) {
  if (!ts.isCallExpression(node)) {
    return false;
  }
