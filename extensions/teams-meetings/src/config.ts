...
export type TeamsMeetingsConfig = {
  enabled: boolean;
  defaultMode: TeamsMeetingsMode;
  chrome: {
    audioBackend: "blackhole-2ch";
    audioFormat: TeamsMeetingsAudioFormat;
  ...
  realtime: {
    strategy: TeamsMeetingsRealtimeStrategy;
    provider?: string;
    ...
    introMessage?: string;
    agentId?: string;
    toolPolicy: TeamsMeetingsToolPolicy;
    providers: Record<string, Record<string, unknown>>;
  };
};

export function resolveTeamsMeetingsGatewayOperationTimeoutMs(config: TeamsMeetingsConfig): number {
  return Math.max(60_000, addTimerTimeoutGraceMs(config.chrome.joinTimeoutMs, 30_000) ?? 1);
}

const DEFAULT_AUDIO_BUFFER_BYTES = 4_096;
const DEFAULT_AUDIO_FORMAT: TeamsMeetingsAudioFormat = "pcm16-24khz";

function buildSoxCommands(format: TeamsMeetingsAudioFormat, bufferBytes: number) {
  return format === "g711-ulaw-8khz"
    ? buildMeetingSoxAudioCommands({
        bufferBytes,
        format: { sampleRate: 8_000, channels: 1, encoding: "mu-law", bits: 8 },
      })
    : buildMeetingSoxAudioCommands({
        bufferBytes,
...
function resolveMode(value: unknown): TeamsMeetingsMode {
...
function resolveAudioFormat(value: unknown): TeamsMeetingsAudioFormat {
...
export function resolveTeamsMeetingsConfig(input: unknown): TeamsMeetingsConfig {
