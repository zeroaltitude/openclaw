...
export function readSupervisedCommandContext(
  executionId: string,
  allocationId: string,
  ...
  if (!operation?.workspaceVersion) {
    throw new Error("Command input version is not pinned");
  }
  ...
  if (
    ...
    throw new Error("Command export lacks its exact runner-owned allocation");
  }
  return {
    execution,
    operation,
