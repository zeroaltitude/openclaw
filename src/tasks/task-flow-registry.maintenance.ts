 ...
 * linked tasks are skipped for the same safety reason the "prune" action in
 * `resolveTaskFlowMaintenanceAction` skips them: they should not normally
 * exist (a flow whose status is terminal has no active linked tasks), so a
 * match here means the status is stale and deleting the record now would be
 * premature.
 *
...
export function clearTerminalTaskFlowsByStatus(status: ClearableTerminalTaskFlowStatus): {
  ...
  const hasActiveLinkedTasks = (flowId: string): boolean =>
    listTasksForFlowId(flowId).some(isTaskFlowCancellationPending);
  let cleared = 0;
  let skipped = 0;
  for (const flow of listTaskFlowRecords()) {
