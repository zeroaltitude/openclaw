...
describe("buildModelsProviderData app-server runtime choices", () => {
  ...
  it("offers the GLM bridge runtime for zai models", async () => {
    const data = await buildModelsProviderData(CONFIG);

    expect(data.runtimeChoicesByProvider?.get("zai")).toEqual([
      {
        id: "openclaw",
        label: "OpenClaw Default",
        description: "Use the built-in OpenClaw runtime.",
      ...
      {
        id: "glm-bridge",
        label: "GLM app-server",
        description: "Use the GLM app-server runtime selected by the effective harness policy.",
      },
    ]);
  });
