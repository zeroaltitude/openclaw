...
export async function persistHeartbeatOutcome(params: {
  ...
  const options = toDatabaseOptions(captureLifecycleDatabaseScope(resolveSqliteScope(params)));
...
export async function claimHeartbeatOutcomeForRun(params: {
  ...
  const { sessionKey, runId, assertCurrent } = params;
  const options = toDatabaseOptions(captureLifecycleDatabaseScope(resolveSqliteScope(params)));
    ...
    () =>
      runOpenClawAgentWriteTransaction(
        ({ db }) => {
          ...
          if (!row || (row.context_run_id !== null && row.context_run_id !== runId)) {
            return undefined;
          }
          if (row.context_run_id === null) {
            const claim = executeSqliteQuerySync(
              ...
              agentDb
                ...
                .set({ context_run_id: runId, context_claimed_at: Date.now() })
                .where("session_key", "=", sessionKey)
