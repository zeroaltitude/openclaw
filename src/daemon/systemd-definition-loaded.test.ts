...
it.skipIf(process.platform === "win32").each([false, true])(
  "inspects selected loaded-only definition authority without loading either manager (legacy=%s)",
  async (legacy) => {
    vi.mocked(execFileUtf8).mockReset().mockImplementation(systemdManagerVersionProbe);
    const root = await fs.realpath(dirs.make("openclaw-definition-loaded-"));
    const env = {
      HOME: path.join(root, "home"),
      XDG_RUNTIME_DIR: path.join(root, "runtime"),
      DBUS_SESSION_BUS_ADDRESS: `unix:path=${root}/bus`,
      OPENCLAW_STATE_DIR: path.join(root, "state"),
      ...(legacy ? { OPENCLAW_PROFILE: "lisa" } : { OPENCLAW_SYSTEMD_UNIT: "openclaw-owned" }),
    };
    const unitName = legacy ? "openclaw-lisa.service" : "openclaw-owned.service";
    const unitPath = path.join(env.HOME, ".config/systemd/user", unitName);
    ...
    if (legacy) {
      // An unrelated canonical backup must not decide the selected legacy unit's authority.
      await fs.symlink(
        unitPath,
        path.join(path.dirname(unitPath), "openclaw-gateway-lisa.service.bak"),
      );
    }
    ...
    await expect(
      readSystemdDefinitionMutationCapability(env, {
        requireLoaded: true,
        timeoutMs: 1000,
        ...(legacy ? { systemdReadTarget: { scope: "user" as const, unitName, unitPath } } : {}),
      }),
    ).resolves.toEqual({ kind: "writable" });
    expect(system).toHaveBeenCalledWith(unitName, expect.any(Number), {
      requireLoaded: true,
    });
    expect(
      busctl.mock.calls
        .filter(([, args]) => args.includes("GetUnit"))
        .every(([, args]) => args.at(-1) === unitName),
    ).toBe(true);
