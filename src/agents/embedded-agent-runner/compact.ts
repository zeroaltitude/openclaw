...
async function compactEmbeddedAgentSessionDirectOnce(
  ...
  await ensureOpenClawModelsJson(params.config, agentDir, {
    ...
    // short-circuit branch can require this model to be on disk before
    // blessing the fast path (Codex P2 round-9 on PR #73261).  Without
    // this hint, an implicit-discovery setup with a stale models.json
    // could short-circuit, then fail in `resolveModelAsync` below with
    // `Unknown model`.
    targetModel: modelId,
  });
