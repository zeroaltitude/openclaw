...
describe("subagent registry lifecycle hardening", () => {
  ...
  // Same terminal-lock/ownership contract, but entered from the sweeper's
  // unconfirmed-child gate: a wait-expiry timeout alone must not settle a run,
  // so this exercises settleSubagentRunFromSessionStore directly against the
  // child's own persisted session-store record instead of the orphan helper.
